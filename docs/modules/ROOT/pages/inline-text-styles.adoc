...
In the default UI, the `<strong>` element is styled in the 500 font weight of the current typeface (Lato).
For example:
...
In the default UI, the `em` element is styled in the italic font variant of the current typeface (Lato).
For example:
...
In the default UI, the `code` element is styled using the fixed-width font loaded by the stylesheet (Inconsolata).
For example:
